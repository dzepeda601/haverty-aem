...
/**
 * Transformer: Havertys Dynamic Media / Scene7 images.
 * Source uses DM Open API (delivery-p48208-e247412.adobeaemcloud.com/adobe/assets/urn:...)
 * and Scene7 IS/Image (havertys.scene7.com/is/image/...) URLs — confirmed in
 * migration-work/metadata.json .images.mapping.
 *
 * Rewrites DM/S7 <img> tags into anchors so the URLs survive the docx→markdown
 * round-trip; a companion auto-block in scripts/scripts.js rebuilds them into
 * responsive <picture> elements at render time. Runs in afterTransform ONLY so
 * block parsers can extract <img> refs into their cells first.
 *
 * See references/dm-scene7-transformer.md — the canonical spec. Helpers below
 * are copied byte-identical from references/dm-scene7-helpers.js (the subset the
